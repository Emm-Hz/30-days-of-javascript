...
const countriesInside = ["Finland", "Estonia", "Sweden", "Denmark", "Norway"];
const rectangle = {
  width: 20,
  height: 10,
  area: 200,
...
function levelTwo() {
  ...
  for (const { name, scires, skills, age } of users) {
    console.log(name, scires, skills, age);
  }
...
function convertArrayToObject(arrayToConvert) {
  ...
  for (const [name, [...skills], [...scores]] of students) {
    structureObject.push({ name: name, skills: skills, scores: [scores] });
  }
...
const copyStudent = { ...studentObject };

copyStudent.skills.frontEnd.push({ skill: "Bootstrap", level: 8 });
